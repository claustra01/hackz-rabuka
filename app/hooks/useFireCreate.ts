...
export const useFireCreate = (sceneRef: RefObject<THREE.Scene | null>) => {
	const fireTex = new THREE.TextureLoader().load("favicon.ico");
	...
	textureLoader.load("favicon.ico", (texture) => {
		...
		fireMaterial.uniforms.fireTex.value = texture;
	});
	// シェーダーコード
	const FireShader = {
		defines: {
	...
	fireMaterial.uniforms.magnitude.value = 7;
	fireMaterial.uniforms.noiseScale.value = new THREE.Vector4(2, 2, 2, 0.3);
	...
	const animate = () => {
		fireMaterial.uniforms.time.value = clock.getElapsedTime() * 10;
